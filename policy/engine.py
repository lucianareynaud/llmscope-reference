...
from policy.loader import load_policy, PolicyConfig
...
class YAMLPolicyEngine:
    ...
    def _evaluate_budget_threshold(
        ...
        try:
            ...
            # Build query based on whether budget_namespace is specified
            if budget_namespace:
                query = f"""
                    SELECT COALESCE(SUM(estimated_cost_usd), 0.0) as total_cost
                    FROM read_json_auto('{telemetry_path}')
                    WHERE timestamp >= '{window_start_iso}'
                      AND budget_namespace = '{budget_namespace}'
                """
            else:
                query = f"""
                    SELECT COALESCE(SUM(estimated_cost_usd), 0.0) as total_cost
